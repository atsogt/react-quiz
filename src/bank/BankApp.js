...
const reducer = (state, action) => {
  switch (action.type) {
    ...
    case ACTIONS.PAYLOAN:
      if (state.balance < state.loan || !state.loan) return { ...state };
  ...
  return (
    <div className="App">
      ...
      <p>
        <button
          onClick={() => {
            dispatch({ type: ACTIONS.OPENACCOUNT });
          }}
          disabled={isActive}>
          Open account
        </button>
      ...
      <p>
        <button
          onClick={() => {
            dispatch({ type: ACTIONS.REQUESTLOAN, payload: 5000 });
          }}
          disabled={!isActive}>
